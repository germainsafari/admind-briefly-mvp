...
interface ManagerDashboardSearchProps {
  className?: string
...
export function ManagerDashboardSearch({ className, onResultClick }: ManagerDashboardSearchProps) {
  ...
  const handleResultClick = (result: SearchResult) => {
    if (onResultClick) {
      ...
      switch (result.type) {
        case 'client':
          router.push(`/manager/clients/${result.id}`)
          break
        case 'brief':
          router.push(`/manager/briefs/${result.id}`)
          break
      }
    }
    setShowResults(false)
    ...
    <div className={cn("relative", className)}>
      <div className="flex items-center space-x-2">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search clients and briefs..."
            value={query}
